...
# Base Configurations
BASE_DIR = "/Users/sohamghadge/Desktop/Projects/work/Portfolio Art Direction"
DOMAIN = "https://dr-dipti.com"
...
def generate_pages():
    ...
    for t_id, t_info in TREATMENTS.items():
        for a_id, a_info in ASPECTS.items():
            ...
            page_title = f"{t_info['name']} {a_info['suffix']} | Dr. Dipti Mathias"
            meta_desc = f"{a_info['description']} {t_info['description']} Available in Khar West, Mumbai."
            ...
            content_html += f"<p>{t_info['description']} Under the care of Dr. Dipti Mathias, each protocol is customized to deliver natural results.</p>"
            ...
            if a_id == "cost":
                ...
                content_html += f"<p>{t_info['procedure']} Safety is our highest priority at Good Genes Dermatology. Common mild reactions include brief redness or swelling.</p>"
            ...
            page_html = page_html.replace("{{ category_link }}", f'<a href="/treatments.html#{t_info["anchor"]}">{t_info["category"]}</a>')
    ...
    for loc_id, loc_name in LOCATIONS.items():
        ...
        loc_title = f"Best Dermatologist in {loc_name} | Dr. Dipti Mathias"
        loc_desc = f"Premium dermatology and aesthetic clinic serving patients in {loc_name}, Mumbai. Advanced skin lifting, Botox, acne treatments, and fillers."
        loc_keywords = f"Dermatologist {loc_name}, skin clinic {loc_name}, dermatologist in {loc_name} Mumbai"
        ...
        loc_content += "<ul>"
        for t_id, t_info in TREATMENTS.items():
            loc_content += f"<li><a href='/treatments/{t_id}/index.html' style='color:#232E21; font-weight:500;'>{t_info['name']}</a> &mdash; {t_info['description']}</li>"
        loc_content += "</ul>"
        ...
        for t_id, t_info in TREATMENTS.items():
            ...
            local_t_title = f"{t_info['name']} in {loc_name} | Good Genes Dermatology"
            local_t_desc = f"Looking for {t_info['name']} in {loc_name}, Mumbai? Visit Dr. Dipti Mathias at Amore Edge, Khar West for customized {t_info['name']} treatments."
            local_t_keywords = f"{t_info['name']} {loc_name}, best {t_info['name']} in {loc_name}"
    ...
    # Add treatments list page
    usr = ET.SubElement(urlset, "url")
    loc = ET.SubElement(usr, "loc")
    loc.text = f"{DOMAIN}/treatments.html"
